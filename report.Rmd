...
knitr::opts_chunk$set(cache = F,warning = FALSE)
...
roc_IU <- function(roc)
{
  ...
  best_IU <- min(IU)
  best_threshold <- roc$thresholds[which(IU==best_IU)]
...
print_roc <- function(test_y,test,model)
{
  ...
  print(model_roc)
  cutpoint <- roc_IU(model_roc)
  print(paste0('cut_point:',cutpoint,'\n'))
  plot(model_roc,main = names(model$coef))
}
```
...
比較過後backward selection 的模型AUC 最大

Goal: 定義各 chem 在各條件下對應的 level

```{r}
# 比較過後backward selection 的模型AUC 最大。
print_roc(chem_test_y,chem_test,level_for_back_model)
```

```{r}
all_glm <- glm(level~caffeine+Catechin+FAA+GCG+EGC+Theanine,family = binomial(),data = chem_train)
print_roc(chem_test_y,chem_test,all_glm)
```

```{r}
caffeine_glm <- glm(level~caffeine,family = binomial(),data = chem_train)
Catechin_glm <- glm(level~Catechin,family = binomial(),data = chem_train)
FAA_glm <- glm(level~FAA,family = binomial(),data = chem_train)
GCG_glm <- glm(level~GCG,family = binomial(),data = chem_train)
EGC_glm <- glm(level~EGC,family = binomial(),data = chem_train)
Theanine_glm <- glm(level~Theanine,family = binomial(),data = chem_train)
polyphenol_glm <- glm(level~polyphenol,family = binomial(),data = chem_train)
GA_glm <- glm(level~Gallic_Acid,family = binomial(),data = chem_train)
```

```{r}
print_roc(chem_test_y,chem_test,caffeine_glm)
print_roc(chem_test_y,chem_test,Catechin_glm)
print_roc(chem_test_y,chem_test,FAA_glm)
print_roc(chem_test_y,chem_test,EGC_glm)
print_roc(chem_test_y,chem_test,GCG_glm)
print_roc(chem_test_y,chem_test,Theanine_glm)
print_roc(chem_test_y,chem_test,polyphenol_glm)
print_roc(chem_test_y,chem_test,GA_glm)
```

```{r}
tune1_glm <- glm(level~caffeine+Catechin+polyphenol,family = binomial(),data = chem_train)
print_roc(chem_test_y,chem_test,tune1_glm)
tune2_glm <- glm(level~caffeine+Catechin,family = binomial(),data = chem_train)
print_roc(chem_test_y,chem_test,tune2_glm)
```

# chem\*\~weather\*

```{r}
chem_target <- c('caffeine','Catechin')
```
...
for (y_name in chem_target)
{
  ...
  #full_model <- formula(paste0(model_gen(weather,1,y_name)))
  for_back_selection(y_name,weather_train,weather_test,weather_test_y,lm )
  print('===============')
}
```

## var selection base on quardratic and interaction
...
```{r all chem quadratic,include=F}
...
{
  print(y_name)
  full_model <- formula(paste0(model_gen(chem,1:2,y_name),'+(.)^2'))
  for_back_selection(y_name,chem_train,chem_test,chem_test_y,lm,full_model = full_model )
  print('===============')
}
```
...
Caffeine,acu_temp is most important

```{r}
rf_dat <- analyze_data %>% select(., c(caffeine,cultivar,rain,acu_mean_temp,RH,Solar_rad_H,Solar_rad_MJM2))
rf_this('caffeine',rf_dat,train,test,train_idx,test_idx)
```
